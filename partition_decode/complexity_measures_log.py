...
class EvaluationMetrics(NamedTuple):
    ...
    all_complexities: Dict[Complexity, float]
...
@contextmanager
def _perturbed_model(model, sigma: float, rng, magnitude_eps: Optional[float] = None):
    ...
    if magnitude_eps is not None:
        ...
        noise = [
            torch.normal(0, sigma ** 2, p.shape, generator=rng).to(device)
            for p in model.parameters()
        ]
    model = deepcopy(model)
    try:
        ...
        for _ in range(montecarlo_samples):
            with _perturbed_model(model, sigma, rng, magnitude_eps) as p_model:
                ...
                for data, target in dataloader:
                    ...
                    batch_correct = (
                        pred.eq(target.data.view_as(pred)).type(torch.FloatTensor).cpu()
                    )
...
def get_all_measures(
    ...
    def _margin(model, dataloader: DataLoader) -> Tensor:
        ...
        for data, target in dataloader:
            ...
            print(logits.shape)
            #   correct_logit = logits[torch.arange(logits.shape[0])].clone()
            #   logits[torch.arange(logits.shape[0])] = float('-inf')
            correct_logit = logits[torch.arange(logits.shape[0]), target].clone()
            logits[torch.arange(logits.shape[0]), target] = float("-inf")

            max_other_logit = logits.data.max(
                1
            ).values  # get the index of the max logits
    ...
    measures[CT.LOG_PROD_OF_SPEC_OVER_MARGIN] = (
        measures[CT.LOG_PROD_OF_SPEC] - 2 * margin.log()
    )  # 31
    measures[CT.LOG_SPEC_INIT_MAIN] = (
        measures[CT.LOG_PROD_OF_SPEC_OVER_MARGIN]
        + (dist_fro_norms / spec_norms).sum().log()
    )  # 29
    measures[CT.FRO_OVER_SPEC] = (fro_norms / spec_norms).sum()  # 33
    measures[CT.LOG_SPEC_ORIG_MAIN] = (
        measures[CT.LOG_PROD_OF_SPEC_OVER_MARGIN] + measures[CT.FRO_OVER_SPEC].log()
    )  # 30
    measures[CT.LOG_SUM_OF_SPEC_OVER_MARGIN] = math.log(d) + (1 / d) * (
        measures[CT.LOG_PROD_OF_SPEC] - 2 * margin.log()
    )  # 34
    measures[CT.LOG_SUM_OF_SPEC] = (
        math.log(d) + (1 / d) * measures[CT.LOG_PROD_OF_SPEC]
    )  # 35

    # Frobenius Norm
    measures[CT.LOG_PROD_OF_FRO] = fro_norms.log().sum()  # 37
    measures[CT.LOG_PROD_OF_FRO_OVER_MARGIN] = (
        measures[CT.LOG_PROD_OF_FRO] - 2 * margin.log()
    )  # 36
    measures[CT.LOG_SUM_OF_FRO_OVER_MARGIN] = math.log(d) + (1 / d) * (
        measures[CT.LOG_PROD_OF_FRO] - 2 * margin.log()
    )  # 38
    measures[CT.LOG_SUM_OF_FRO] = (
        math.log(d) + (1 / d) * measures[CT.LOG_PROD_OF_FRO]
    )  # 39
    ...
    def _path_norm(model) -> Tensor:
        ...
        for param in model.parameters():
            if param.requires_grad:
                param.data.pow_(2)
        x = torch.ones(
            [1] + list(dataset_type_D), device=device
        )  # modified line, replaced model.dataset_type.D with dataset_type_D
        x = model(x)
        del model
        return x.sum()
    ...
    def _spectral_norm_fft(kernel: Tensor, input_shape: Tuple[int, int]) -> Tensor:
        ...
        transforms = np.fft.fft2(
            np_kernel, input_shape, axes=[0, 1]
        )  # Shape(ih,iw,in,out)
        singular_values = np.linalg.svd(
            transforms, compute_uv=False
        )  # Shape(ih,iw,min(in,out))
    ...
    def _pacbayes_bound(reference_vec: Tensor) -> Tensor:
        return (
            (reference_vec.norm(p=2) ** 2) / (4 * sigma ** 2) + math.log(m / sigma) + 10
        )
    ...
    def _pacbayes_mag_bound(reference_vec: Tensor) -> Tensor:
        numerator = (
            mag_eps ** 2 + (mag_sigma ** 2 + 1) * (reference_vec.norm(p=2) ** 2) / omega
        )
        denominator = mag_eps ** 2 + mag_sigma ** 2 * dist_w_vec ** 2
        return (
            1 / 4 * (numerator / denominator).log().sum() + math.log(m / mag_sigma) + 10
        )
