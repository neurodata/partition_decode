...
# Original Author: Isabelle Guyon and Arthur Pesah, ChaLearn, August-November 2014
...
def convert_to_num(Ybin, verbose=False):
    ...
    if verbose:
        print(Ybin.shape, classid.shape, Ycont.shape)
    return Ycont
...
def normalize_array(solution, prediction):
    ...
    maxi = np.nanmax(
        (filter(lambda x: x != float("inf"), sol))
    ...
    new_prediction[
        new_prediction > 1
    ] = 1  # and if predictions exceed the bounds [0, 1]
...
if __name__ == "__main__":
    ...
    Pred.append(
        np.array(
            [[0.75, 0.25, 0.0], [0.75, 0.25, 0.0], [0.75, 0.25, 0.0], [0.75, 0.25, 0.0]]
        )
    )
    Sol.append(sol2)
