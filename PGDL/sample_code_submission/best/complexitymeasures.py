...
def complexityDB(
    ...
    db_score = {}
    C = CustomComplexityFinal(
        model, dataset, augment="mixup", computeOver=computeOver, batchSize=batchSize
    )
...
def complexityMixup(model, dataset, program_dir=None, computeOver=500, batchSize=50):
...
def complexityManifoldMixup(
    model, dataset, program_dir=None, computeOver=1000, batchSize=50
):
    ...
    def multiplicativeNoise(img, std=2):
        return img * (tf.random.normal(mean=1.0, stddev=std, shape=img.shape, seed=1))

    def veracityRatio(model, batches, label, layer=0, version_loss=False):

        cloned_model = keras.models.clone_model(
            model, input_tensors=keras.Input(shape=(batches[0][0][0].shape))
        )
...
def complexityMixupSoft(
    model, dataset, program_dir=None, computeOver=1000, batchSize=100
):
