...
class CustomComplexityFinal:
    ...
    def __init__(
        ...
        if rootpath is not None:
            self.rootPath = os.path.join(
                rootpath, "computed_data/pickles/{}".format(mid)
            )
    ...
    def computeMargins(self, top=2):
        ...
        it = iter(
            self.dataset.repeat(-1).shuffle(5000, seed=self.seed).batch(self.batchSize)
        )
        marginDistribution = {}
        totalVariance = {}
        totalVarianceTensor = {}
        ...
        for i in range(self.computeOver // self.batchSize):
            ...
            for layer in self.layers:
                ...
                try:
                    ...
                    totalVarianceTensor[layer] = np.vstack(
                        (
                            totalVarianceTensor[layer],
                            np.array(inter).reshape(inter.shape[0], -1),
                        )
                    )
                except Exception as e:
                    marginDistribution[layer] = grads
                    totalVarianceTensor[layer] = np.array(inter).reshape(
                        inter.shape[0], -1
                    )
        ...
        for layer in self.layers:
            totalVariance[layer] = (
                trim_mean(
                    np.var(
                        totalVarianceTensor[layer].reshape(
                            totalVarianceTensor[layer].shape[0], -1
                        ),
                        axis=0,
                    ),
            ...
            marginDistribution[layer] = np.array(marginDistribution[layer]) / (
                np.array(totalVariance[layer]) + 1e-7
            )  # /np.sqrt(m_factor) #/totalVarianceTensor[layer].shape[1:][0]
    ...
    def distancefromMargin(self, batch, layer, top=2):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            out_hard = tf.math.top_k(tf.nn.softmax(intermediateVal[-1], axis=1), k=top)[
                1
            ]
            ...
            if self.penalize:
                ...
                top_1 = top_1_og * mask + batch[1] * (1.0 - mask)
                top_2 = top_2_og * mask + top_1_og * (1.0 - mask)
            else:
                top_1 = out_hard[:, top - 2]
                top_2 = out_hard[:, top - 1]
                mask = np.array(top_1 == batch[1]).astype(int)
                top_2 = top_2 * mask + batch[1] * (1.0 - mask)
            ...
            if self.network_scale == True:
                ...
                numerator = (
                    tf.gather_nd(preds, tf.constant(index1, tf.int32))
                    - tf.gather_nd(preds, tf.constant(index2, tf.int32)).numpy()
                )
                grad_diff = np.reshape(
                    grad_i.numpy(), (grad_i.numpy().shape[0], -1)
                ) - np.reshape(grad_j.numpy(), (grad_j.numpy().shape[0], -1))
                denominator = np.linalg.norm(grad_diff, axis=1, ord=2)
    ...
    def batchMixup(self, batch, seed=1):
        ...
        for i in range(np.max(batch[1])):
            lam = (np.random.randint(0, 3, size=x[batch[1] == i].shape[0]) / 10)[
                ..., None, None, None
            ]
            mix_x[(batch[1] == i)] = x[batch[1] == i] * lam + tf.random.shuffle(
                x[batch[1] == i], seed=seed
            ) * (1 - lam)

        if self.augment == "mixup":
            mix_x = mix_x[
                np.random.randint(0, mix_x.shape[0], size=self.batchSize // 3)
            ]
    ...
    def batchMixupLabelwiseLinear(self, batch, seed=2):
        ...
        edges = np.array(
            [
                len(sorted_labels[sorted_labels == i])
                for i in range(max(sorted_labels) + 1)
            ]
        )
        ...
        return (
            tf.convert_to_tensor(intrapolateImages),
            tf.convert_to_tensor(sorted_labels),
        )
