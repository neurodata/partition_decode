...
#   rng = rng.to(device)
  if magnitude_eps is not None:
    noise = [torch.normal(0, sigma**2 * torch.abs(p) ** 2 + magnitude_eps ** 2, generator=rng).to(device) for p in model.parameters()]
  else:
...
def _pacbayes_sigma(
  ...
  device = next(model.parameters()).device
  print(device)
  rng = torch.Generator(device=device) if magnitude_eps is not None else torch.Generator(device=device)
